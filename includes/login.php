...
class Keyy_Login {
	...
	public function authenticate($user, $login, $password) {
		...
		// Prevent incorrect messages on the login screen
	...
	public function get_user_login_policy($user_id) {
		...
		$is_configured_for_user = $this->is_configured_for_user($user_id);
		
		// Possible values: required, sufficient, ignored
		// Not all combinations make sense, and hence not all are supported.
		// The default policy (i.e. in the absence of any over-ride) is that if the user has set it up, then they must use it; otherwise no change (they must use their password)
		$policy = array(
			'keyy' => $is_configured_for_user ? 'required' : 'ignored',
			// 'ignored' will cause processing to be aborted and default WP handling to continue
			'password' => $is_configured_for_user ? 'ignored' : 'required',
		);
	 ...
	 */
	public function set_user_settings($settings, $user_id = false) {
	...
	public function get_fresh_login_token($params = array()) {
		...
		while (!$key_available) {
			...
			if (false === get_option($options_table_key)) {
				$key_available = true;
			} else {
				$tries++;
				if ($tries > 19) {
					// This is taking surprisingly long. Purge old tokens in case the space is full from a brute-force attack (and render such attacks, even if they could run at 200 requests/second without melting the web-server, still certain to eventually fail, as a legitimate log-in has equal opportunity to grab a new token).
					
					$age = min(106400 - $tries * 1000, 300);
					
					$this->prune_expired_login_tokens($age);
				}
			}
	 ...
	 * A good time to run this is after a successful login.
	 *
	 * @param Integer $age - expire unused tokens after this amount of time
	 */
	public function prune_expired_login_tokens($age = 86400) {
		...
		$sql = "
				...
				AND b.option_value < (UNIX_TIMESTAMP() - $age)
			";
