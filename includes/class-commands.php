...
class Keyy_Commands {
	 ...
	 * @var array
	 */
	private $user_actions = array('ping', 'get_version', 'get_connection_token', 'save_settings', 'get_fresh_login_token', 'get_dashboard_page', 'get_disconnection_result', 'dismiss_ua_notice', 'get_connection_result', 'get_token_state', 'disconnect', 'login', 'connect', 'logout');
	...
	public function get_version() {
		...
		return array(
			'plugin_version' => Keyy_Login_Plugin::VERSION,
			'app_compat_version' => Keyy_Login_Plugin::APP_COMPAT_VERSION
		);
	}
	...
	/**
	 * Dismisses the admin notice notifying the user of an app update for 2 weeks
	 *
	 * @param  Array $params - parameters. These are passed on to Keyy_Login::dismiss_ua_notice.
	 * @return Boolean - The sucess/failure status of the update.
	 */
	public function dismiss_ua_notice($params = array()) {

		$user = wp_get_current_user();

		if (0 == $user->ID) return false;

		$user_agents = $this->keyy->get_user_agent($user->ID);

		if (!is_array($user_agents)) $user_agents = array();
			
		// Push the last seen forward by 2 weeks
		foreach ($user_agents as $agent => $time_seen) {
			$user_agents[$agent] = time() + 86400 * 7 * 2;
		}

		return update_user_meta($user->ID, Keyy_Login_Plugin::META_KEY_USER_AGENTS, $user_agents);
	}
	 ...
	 */
	 public function login($params) {
		...
		// If user didn't logout properly on mobile device and relogin attempt is made via the Keyy app, logout sequence should be first initiated before logging user in again to avoid showing Username password fields. 
